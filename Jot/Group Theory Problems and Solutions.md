...
$K$ is the Klein four-group, a subgroup of *S₄*.  To show $K \trianglelefteq S_4$, we use the normality test.  We need to show that $\sigma K \sigma^{-1} \subseteq K$ (or equals K) for all $\sigma \in S_4$. Since transpositions generate $S_4$, we only need to check for transpositions. Let $\sigma = (a\;b)$ be a transposition.

Conjugating elements of $K$ by $(ab)$ amounts to swapping $a$ and $b$ in the cycle notation (Lemma 3 §2.8). Since all elements of $K$ are products of disjoint transpositions, swapping any two elements will result in another element of $K$.  Therefore, $K \trianglelefteq S_4$.

$|S_4/K| = |S_4|/|K| = 24/4 = 6$. There are two groups of order 6 up to isomorphism: $C_6$ (cyclic) and $S_3$.

The cosets of K in S₄ are:
* K
* K(1 2 3)
* K(1 3 2)
* K(1 2)
...
(Calculating the full Cayley table is tedious). Let α = K(1 2 3) and β = K(1 2). Then α³=K, β² = K, and αβα = β. This is a presentation for S₃. Thus $S_4/K \cong S_3$.